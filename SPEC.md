...
一隻 5–10 分鐘完成一局嘅文字模擬經營遊戲：玩家由 20 歲開始，用 40 年靠炒貨、創業同家庭選擇，喺 60 歲退休前累積至少 $100,000,000。
...
- 所有玩家可見文字用繁體中文、偏香港口語；數字顯示使用 $ 同千位分隔。
...
遊戲唔係嚴肅理財工具，而係一隻帶香港生活感嘅惡搞模擬遊戲。所有畫面要做到「一眼睇得明、兩眼覺得荒謬、三眼想繼續玩」。
...
- 事件 modal：大標題、角色/商品插畫位、短句效果摘要；動畫只作 feedback，唔可以阻礙玩家快速完成一局。
...
8. 主要畫面及事件呈現一致嘅惡搞style，而唔係純文字表格或一般 admin dashboard。
...
- `event` 未 `chooseEvent` 前，禁止所有市場及投資操作。
...
| 起始名聲 | 0，範圍 0–100                       |
| 起始倉庫 | 100 格                              |
| 遊戲目標 | 退休時總資產 >= $100,000,000        |
| 貨幣     | 整數 $；所有計算最後 round 到整數   |
...
| id             | 名稱   | 起始現金                                      |
| -------------- | ------ | --------------------------------------------- |
| `low_class`    | 死窮撚 | $45,000（起步名聲 5；首次創業／首次擴倉折扣） |
| `middle_class` | 死中產 | $100,000                                      |
| `high_class`   | 二世祖 | $1,000,000                                    |

投胎結果必須寫入首條 log（例如「投胎成功：你而家係一名死窮撚，起步資金 $25,000」），並喺開局時讓玩家睇到。固定 seed 時必須可重現同一家庭。
...
1. `beginTurn`：以目前年齡計算通脹，產生本年商品價格（未含事件 multiplier）及一個隨機事件。
2. 進入 `event`：事件只揭示情境，**未套用效果**；玩家必須揀一個 choice。
3. `chooseEvent`：先套用該 choice 嘅效果（改價、現金、健康、名聲），再進入 `playing`。事件資料必須 data-driven。
...
price = round(basePrice * inflation * random(0.7, 1.45) * eventMultiplier)
```

`random(0.7, 1.45)` 要由可注入 RNG 產生；唔可以喺 render 時重新計價。事件 multiplier 預設 1。Phase 3 收窄波幅，避免純炒貨碾壓其他路線。
...
- 買入後倉庫用量不可超過容量。
- 每件商品佔用 `space` 格（見商品 data；低/中階通常 1，高風險貨可更高）；`usedWarehouse = sum(inventory[g] * space[g])`。
- 初始倉庫 100 格；每次升級 +50 格，成本由 data/constants 統一管理。
- 買入、賣出成功及失敗都要寫入 log，方便玩家理解發生咩事。

## 8. 創業系統

MVP 至少有四種公司：珍珠奶茶店、夾公仔樂園、電競館、幽靈廚房、補習天王中心、風水改運館、直播帶貨帝國、AI 科技初創、加密貨幣交易所、停車場王國、房地產集團。公司 definition 最少包括：初始投資、每年收入、每年維護費、估值、最低名聲、倒閉率。
...
事件係**決策點**，唔係純通知。Definition 至少包括 `id`、標題、內文、權重、`choices[]`（每個 choice 有 `id`、`label`、`effects[]`）。

- `beginTurn` 只 roll 事件同基礎市價；**唔套用** effects。
- 玩家喺 `event` phase 必須 `chooseEvent(choiceId)`；套用該 choice 嘅 effects 後先入 `playing`。
- 每個事件最少 2 個 choice（「冇事發生」類都可以有躺平／偷做／社交等分支）；第一個 choice 可視為 auto-dismiss / balance sim 預設路徑。
- 效果類型：`price_mult`（乘以當年已生成價格）、`cash`、`health`、`reputation`。
- 事件效果可以疊加當年通脹，但唔可以改動歷史持倉成本。
- 事件 modal 必須顯示：標題、內文、可揀選項；未抉擇前唔可以靠關閉跳過。效果喺揀完之後先顯示。
...
顯示遊戲名、一句目標、惡搞主視覺、開始按鈕、版本/MVP 標示。唔放長篇教學；第一次操作時用短提示。按開始後，必須先揭示今局投胎家庭同起步資金，再進入第一年事件。
...
chooseEvent(state: GameState, choiceId: string): GameState
dismissEvent(state: GameState): GameState // 自動揀第一個 choice（測試／sim）
...
- 用固定 seed 跑至少 10 局，記錄 20、40、60 歲資產分布（見 `src/game/balance.sim.ts`）。
- 調整價格波幅、公司回報、事件權重，令「全靠一種策略」唔係唯一解。
- 驗證普通玩家一局約 5–10 分鐘完成（40 個回合操作；唔再加長結算流程）。

Phase 3 平衡基線（可重跑 `pnpm exec vitest run src/game/balance.sim.test.ts`）：

- 價格隨機區間：低／中階 `0.62–1.55`；高階（bitcoin／gold／ev／options）`0.66–1.50`（壓低純 yolo 尾段爆炸）。
- 高階貨倉庫：bitcoin／options 各 2 格，ev 3 格，gold 1 格。
- 平淡事件權重提高；極端炒價／現金大獎幅度下調。
- 公司淨年回報約 15–22%；早期舖（奶茶／夾公仔）略平，方便窮撚入局；估值約 1.35–1.5× 投資額。
- 創業開業 IPO：當刻市值 = 實際投資額，唔可以即刻賣盤賺差價；股價年結先按估值浮動。
- 股價年結 roll：`0.82–1.28`；倉庫升級 base `$45k`、growth `1.75`；死窮撚首次擴倉 4 折。
- 死窮撚：起步 $45k、名聲 5、首次創業 7 折。
- 可玩性補強：負面現金事件封頂（最多 50% 現金、保留 $1,000）、健康年耗 5、五年免費檢查 +10、醫生基價／wealth rate 提高、第一次創業保底成功、失敗退一半籌備費。
- 標題頁可選「簡易模式」（健康消耗減半、負面現金事件再半）。
- UX：市場合約價格訊號、快捷數量、結束回合僅危險時 confirm、年結摘要 modal、資產／人生里程碑 toast。
- 模擬：≥20 fixed seed × 5 策略；總勝率約 3–25%；trade / company 各有 ≥$10M；yolo 唔可以 100% almost；死窮撚至少有 almost 局；報表分策略 + 分家庭。
...
- `vite-plugin-pwa`：production build 產生 service worker；`main.tsx` 用 `registerSW({immediate: true})`，`onNeedRefresh` 自動 `location.reload()` 強制用新版；離線可用 app shell，遊戲 state 仍唔持久化（zustand localStorage 除外）。
- Icons：`public/icons/icon-192.png`、`icon-512.png`、`apple-touch-icon.png` + SVG；`theme-color` / `background_color` = `#fff8e7`。
- 320px 佈局：`overflow-x: hidden`、safe-area padding、標題／結算貼紙收斂，避免橫向溢出。
