...
Phase 3 平衡基線（可重跑 `pnpm test -- src/game/balance.sim.test.ts`）：

- 價格隨機區間：`0.7–1.45`（由 `0.5–2.0` 收窄）。
- 平淡事件權重提高；極端炒價／現金大獎幅度下調。
- 公司淨回報同估值上調，倒閉率略降，令創業路線可同炒貨競爭。
- 電動車佔 2 倉庫格，令高階梭哈要付出倉位代價。
- 可玩性補強：負面現金事件封頂（最多 50% 現金、保留 $1,000）、健康年耗 4、五年免費檢查、醫生費 cap $50,000、第一次創業保底成功、失敗退一半籌備費。
