...
export const TurnSummaryModal = ({summary, onDismiss}: Props) => {
    ...
    return (
        <Modal onClose={onDismiss} labelledBy="turn-summary-title" closeLabel="關閉年結">
            <p className="text-xs font-black tracking-wide text-(--coral) md:text-base">年結速報</p>
            ...
            {closures.length > 0 ? (
                <div className="mt-4 rounded-2xl border-4 border-(--danger) bg-[#ffe4e6] px-3 py-3 shadow-[3px_3px_0_var(--border)]" role="alert">
                    <p className="text-sm md:text-base font-black text-(--danger)">公司結業通知</p>
                    <ul className="mt-2 space-y-1.5 text-xs font-bold text-(--ink) md:text-base">
                        {closures.map(c => (
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm md:text-base font-bold">
                <div className="rounded-xl border-2 border-(--border) bg-(--bg) px-3 py-2">
                    <p className="text-[10px] md:text-base text-(--muted)">現金變化</p>
                ...
                <div className="rounded-xl border-2 border-(--border) bg-(--bg) px-3 py-2">
                    <p className="text-[10px] md:text-base text-(--muted)">健康變化</p>
                ...
                <div className="col-span-2 rounded-xl border-2 border-(--border) bg-(--bg) px-3 py-2">
                    <p className="text-[10px] md:text-base text-(--muted)">公司淨收入（本回合）</p>
                    <p className="tabular-nums">{formatMoney(summary.companyNet)}</p>
                </div>
            </div>

            {summary.highlights.length > 0 ? (
                <ul className="mt-3 max-h-40 space-y-1 overflow-y-auto rounded-xl border-2 border-(--border) bg-white px-3 py-2 text-xs font-bold md:text-base">
